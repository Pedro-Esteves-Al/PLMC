...
Critério de seleção de pais = torneio
Crossover = sortear 2 pais (padrão - manter)
            filho recebe 1/3 do pai1
...
Mutação =   sortear 1 pai (padrão - manter)
            sortear 1 fazenda para remover no filho
            e ajustar
'''

TAMANHO_POPULACAO = 200
NUMERO_DE_PAIS = int(TAMANHO_POPULACAO*0.1)
TAXA_DE_MUTACAO = int(TAMANHO_POPULACAO*0.15)
TAXA_DE_CROSSOVER = int(TAMANHO_POPULACAO*0.75)
CRITERIO_DE_PARADA = TAMANHO_POPULACAO
...
distF = [[ 0.0,  3.6,  16.5,  28.4,  14.8,  29.1,  25.3,  25.6,  25.1,  22.5,  23.0,  14.6,  17.0,  19.2,  25.6,  25.8,  29.1,  37.5,  27.0,  33.1,  39.8,  41.9,  33.2,  36.2,  37.6,  35.7,  34.9,  36.8,  35.4,  36.3,  38.1,  42.0,  43.4,  43.1,  40.3,  42.4,  47.5,  46.5,  41.6,  50.5],
...
# Gerando a população inicial aleatória : tam 200
while (cont < TAMANHO_POPULACAO):
    cont+=1
...
while(cont < CRITERIO_DE_PARADA): #gerações da população
   ...
   # Passo 1: Separar a lista em 20 grupos de 10 itens cada
    ...
    for i in range(TAXA_DE_CROSSOVER):
        # Seleciona dois pais diferentes aleatoriamente
        pais_indices = random.sample(range(20), 2)
        pai1 = pais[pais_indices[0]][1]
        pai2 = pais[pais_indices[1]][1]
        
        # Cria filho inicial vazio
        filho = []
        
        # Calcula quantos genes pegar do primeiro pai (1/3)
        genes_pai1 = max(1, len(pai1) // 3)
        
        # Adiciona genes aleatórios do pai1 mantendo a ordem
        genes_selecionados = random.sample(range(len(pai1)), genes_pai1)
        genes_selecionados.sort()  # Mantém a ordem original dos genes
        for idx in genes_selecionados:
            filho.append(pai1[idx])
        
        # Função auxiliar para verificar se adicionar um gene melhora a cobertura
        def melhora_cobertura(gene, solucao_atual):
            cobertura_atual = calculaCobertura(solucao_atual)
            solucao_teste = solucao_atual + [gene]
            nova_cobertura = calculaCobertura(solucao_teste)
            return nova_cobertura > cobertura_atual
        
        # Adiciona genes do pai2 que melhoram a cobertura
        for gene in pai2:
            if gene not in filho and melhora_cobertura(gene, filho):
                filho.append(gene)
        
        # Ajusta a solução para garantir cobertura total se necessário
        cobertura_atual = calculaCobertura(filho)
        if cobertura_atual < 40:
            # Cria lista de fazendas não utilizadas
            fazendas_disponiveis = [f for f in range(totF) if f not in filho]
            random.shuffle(fazendas_disponiveis)  # Aleatoriza a ordem
            
            # Tenta adicionar fazendas até atingir cobertura total
            for fazenda in fazendas_disponiveis:
                if calculaCobertura(filho) < 40 and melhora_cobertura(fazenda, filho):
                    filho.append(fazenda)
        
        # Calcula aptidão do filho
        redundancia = calculaRedundancia(filho)
        num_antenas = len(filho)
        aptidao = redundancia / num_antenas
        
        # Adiciona filho à população
        pop.append([aptidao, filho])
   ...
   # Mutação (30 indivíduos)
    for i in range(TAXA_DE_MUTACAO):
        # Seleciona um pai aleatoriamente
        ppai1 = random.randint(0, 19)
        pai1 = pais[ppai1][1][:]  # Cria uma cópia do pai
        
        # Remove uma fazenda aleatoriamente
        posicao_remover = random.randint(0, len(pai1) - 1)
        fazenda_removida = pai1[posicao_remover]
        del pai1[posicao_remover]
        
        # Identifica quais fazendas estão descobertas após a remoção
        cobertura_atual = []
        for j in range(totF):
            coberta = False
            for pos in pai1:
                if distF[pos][j] <= raioA:
                    coberta = True
                    break
            cobertura_atual.append(coberta)
        
        # Identifica fazendas que ficaram descobertas
        fazendas_descobertas = [f for f in range(totF) if not cobertura_atual[f]]
        
        # Se existem fazendas descobertas, ajusta a solução
        if fazendas_descobertas:
            # Lista de fazendas que podem ser adicionadas (exceto a que foi removida)
            candidatas = [f for f in range(totF) if f not in pai1 and f != fazenda_removida]
            
            # Para cada fazenda descoberta, encontra a melhor fazenda candidata para cobri-la
            while fazendas_descobertas:
                melhor_candidata = -1
                max_cobertura_adicional = 0
                
                # Avalia cada fazenda candidata
                for candidata in candidatas:
                    cobertura_adicional = 0
                    for fazenda in fazendas_descobertas:
                        if distF[candidata][fazenda] <= raioA:
                            cobertura_adicional += 1
                    
                    if cobertura_adicional > max_cobertura_adicional:
                        max_cobertura_adicional = cobertura_adicional
                        melhor_candidata = candidata
                
                # Adiciona a melhor candidata encontrada
                if melhor_candidata != -1:
                    pai1.append(melhor_candidata)
                    candidatas.remove(melhor_candidata)
                    
                    # Atualiza lista de fazendas descobertas
                    fazendas_descobertas = [f for f in fazendas_descobertas 
                                        if distF[melhor_candidata][f] > raioA]
                else:
                    break
    
        # Calcula aptidão do filho mutado
