...
txMut = 20
txCro = 70
Critério de seleção de pais = roleta
Crossover = sortear 2 pais (padrão - manter)
            filho recebe 1a metade do pai1
...
Critério de parada = 100 (quantas populações serão geradas)
'''
...
# Gerando a população inicial aleatória : tam 100
while (cont<100):
    cont+=1
...
cont=0
while(cont<100): #gerações da população
    #Roleta
    cont+=1
    faixa=[]
    ini=0
    ...
    pais=[]
    for i in range (10): #10 pais
        pai=random.randint(0,fim)
    ...
    #crossover (70 indivíduos)
    for i in range (70):
        ppai1=random.randint(0,9)
        pai1=pais[ppai1][1]
        ppai2=random.randint(0,9)
        while (ppai1==ppai2):
            ppai2=random.randint(0,9)
        pai2=pais[ppai2][1]
        pos=len(pai1)//2
        filho=[]
        #primeira metado - pai1
