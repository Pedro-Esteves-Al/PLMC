...
#constantes
populacao = 100
numeroDePais = 0.1*populacao
#criterioDeSelecao = aleatorio,eletismo,roleta ou torneio. A márcia escolheu roleta. Não podemos escolher roleta.
taxaDeCrossover = 0.7*populacao
#a forma de fazer crossover não pode ser igual da marcia
#crossover = sortear 2 pais (essa parte tem que ter, pois é padrão, daqui pra baixo tem que mudar)
             # filho recebe 1ª metade do pai1
             # completado com elementos do pai2 até a maxima cobertura
             # se necessário ajustar pra cobrir as fazendas não cobertas
taxaDeMutacao = 0.2 * populacao 
#Mutação = 1 pai gera 1 filho. Sortear 1 pai (essa parte tem que ter pois é padrão)
            #sorteia duas fazendas para inserir no filho
            # e eliminar as fazendas redudantes (exceto as inseridas)
#Critério de parada = quantas populaçoes serão geradas
...
#Se tiver com cobertura ficará com 1, se não, com 0
#cobertura é o total de fazendas atendidas
...
Best=[]
totBest=99999
faBest=0  #função de aptidão da melhor solulçao   
pop = []

#gerando a população inicial aleatória
while (cont<100): #100 porque é a população definida lá em cima
    cont+=1
    ...
    while(tot<40):
        ...
        Sol.append(faz)
        tot=calculaCobertura(Sol)
        #verifica se amplia cobertura, se não ampliar, remove da solução
        if tota==tot:
            i=Sol.index(faz)
            del Sol[i]
            
    red = calculaRedundancia(Sol)
    numeroDeAntenas = len(Sol)
    
    #função de aptidão
    fa = red/tot #quer maximizar a redudancia e minizar o número de antenas.
    pop.append([fa,Sol])
...
#Gerações populacionais (laço, número de pai vezes repetido)

cont = 0

while (cont < 1): #gerações da população (critério de parada)
    #roleta
    cont+=1
    #faixa da roleta
    faixa = []
    ini = 0
    #Seleção dos pais (10 por roleta)
    for i in range (len(pop)):
        numeroDeAntenas = len(pop[i][1])
        fim = ini + 40 -numeroDeAntenas
        faixa.append([ini, fim])
        
        ini = fim + 1
        
    pais = []
    for i in range(10): #10 porque são 10 pais.
        #sorteia um número
        pai = random.randint(0,9) #10 porque são 10 pais
    ...
    #nova população:
        #joga a população antiga fora.
        #pega os pais
        #completa com crossover
        #completa com mutação
        pop = pais[:]
        
    #crossover (70 individuos)
    for i in range(70):
        posicaoPai1=random.randint(0, 9) #0 a 9 pois são 10 pais
        pai1=pais[posicaoPai1][1]
        posicaoPai2=random.randint(0, 9)
        while (posicaoPai1==posicaoPai2):
            posicaoPai2 = random.randint(0, 9)
        pai2 = pais[posicaoPai2][1]
        pos = len(pai1)//2 #duas barras dá o quosciente inteiro
        filho = []
        #primeira metade do pai1
        for j in range (pos):
            filho.append(pai1[j])
            
        #completar com pai2 que melhora a cobertura
        for j in range(len(pai2)):
            ...
            #verifica se amplia cobertura, se não ampliar, remove da solução
            if tota==tot:
                pos=filho.index(candidato)
                del filho[pos]
        
        #Ajuste
        tot = calculaCobertura(filho)
        if tot<40:
            for j in range(len(totF)):
                tota=calculaCobertura(filho)
                filho.append(j)
                tot=calculaCobertura(filho)
                #verifica se amplia cobertura, se não ampliar, remove da solução
    ...
    for i in range(20):
        ...
        #inserindo 2 fazendas
        k=0
        while (k<2):
            #sorteia uma fazenda
            fazenda1 = random.randint(0, totF-1)
            achou = 0
            #procura no pai se tem a fazenda
            for j in range(len(pai1)):
                if pai1[j]==fazenda1:
                    achou=1
                    break
            if (achou==0):
                pai1.append(fazenda1)
                k+=1
                if (k==1):
                    faz1 = fazenda1
                else:
        ...
        numeroDeAntenas=len(pai1)
        fa=red/numeroDeAntenas
        pop.append([fa, pai1])
        if (fa>faBest):
            faBest=fa
            Best=Sol[:]
